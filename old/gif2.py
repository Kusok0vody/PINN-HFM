"""
GIF 2: Для презентации — плавный sweep по значениям beta.
Каждый кадр: PINN prediction | Exact | Error  для одного beta.
Заголовок с beta, L2 ошибкой. Красивый стиль.
"""
...
# Плавный sweep: от 1 до 20 с шагом 0.5 + пауза на крайних значениях
BETAS_SWEEP = (
    [1.0] * 3
...
# ── Глобальные пределы (по всем beta в sweep) ─────────────────────────────────
u_min_global =  1.0 - 1.0   # sin ∈ [-1,1], u = 1 + sin(...)
u_max_global =  1.0 + 1.0
err_max_global = 0.5  # зафиксируем масштаб ошибки

# ── Предсказание для одного beta ──────────────────────────────────────────────
...
# Кэш
print(f"Вычисление {len(set(BETAS_SWEEP))} уникальных beta...")
cache = {}
for b in sorted(set(BETAS_SWEEP)):
    cache[b] = compute_frame(b)
    print(f"  β={b:.1f}  L2={cache[b][3]:.4f}", end="\r")
print(f"\nГотово.")
...
for ax, title in zip(
    ...
    ["Предсказание PINN", "Точное решение", "Модуль ошибки"]
):
...
def update(frame_idx):
    ...
    suptitle.set_text(rf"1D конвекция  $\beta = {beta:.1f}$  |  шаг {step}")
