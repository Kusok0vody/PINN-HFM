...
def make_param_sweep_gif(
    ...
    par_tensor   = physics.par.tensor
    param_names  = physics.par.names
    N_params     = par_tensor.shape[0]
    device       = next(net.parameters()).device

    frames = []
    for i in range(N_params):
        row  = par_tensor[i : i + 1].to(device)
    ...
    style_ax(ax_pred, "PINN prediction")
    ...
    if has_exact:
        ...
        style_ax(ax_exact, "Exact solution")
        ax_exact.set_yticks([])
        ...
        style_ax(ax_err, "Absolute error")
        ax_err.set_yticks([])
    ...
    plt.close(fig)
    print(f"Saved: {output_path}  ({N_params} steps)")
...
def title_fn(param_dict):
    return rf"1D convection   $\beta = {param_dict['beta']:.2f}$   |   step {step}"
