...
print(f"Found checkpoints: {len(steps)}  ({steps[0]} ... {steps[-1]})")
...
print(f"\nDone. Generating GIF ({len(steps)} steps)...")
...
def update(frame_idx):
    s = steps[frame_idx]
    im.set_data(cache[s])
    title.set_text(f"Step {s:>5d}/{MAX_STEP}")
    return [im, title]
...
ax2.set_title(
    rf"PINN: Laplace equation in the ring, step {steps[-1]}",
    fontsize=12, pad=10,
)
